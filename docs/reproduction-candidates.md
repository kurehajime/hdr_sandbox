...
4. （`--extended`指定時）追加切り分け候補を生成
   ...
   - `candidate_probe_alpha_gradient_rl.png`（新規・右→左グラデーション）
   - `candidate_probe_alpha_gradient_tb.png`（新規・上→下グラデーション）
   - `candidate_probe_position_quadrant_alpha64.png`（新規・4象限同一パッチ）
...
- 2026-03-08 v23 追加:
  ...
  - `alpha_gradient` 系の「右だけ光る」現象が位置依存か勾配依存かを追加切り分け
- 2026-03-08 v22 追加:
  - `mapping_conflicts_candidate`（同一candidateに複数fileが紐づく不整合）を検出
  - `mapping_conflicts_file`（同一fileを複数candidateが参照する不整合）を検出
  - `--strict-mapping` で candidate↔file 対応不整合をCI失敗扱いにできる
  - JSONレポートに `mapping_conflicts` セクションを追加
- 2026-03-08 v21 追加:
...
- 2026-03-08 v19 追加:
  ...
  - `generated/alpha_gradient_orientation_spec.md` を追加し、向き差分の判定条件を明文化
...
- `alpha_gradient` / `alpha_gradient_rl` / `alpha_gradient_tb` で、
  「alpha値依存の境界」か「左右固定の位置バイアス」かを切り分ける
  - 向き定義と判定条件は `generated/alpha_gradient_orientation_spec.md` を参照
- `position_quadrant_alpha64` で、4象限同一条件の発光差（画面位置依存）を直接確認する
  - 観測仕様は `generated/position_quadrant_spec.md` を参照
